...
> [!CAUTION]
>
> 同样的 mermaid 代码，https://mermaid-live.nodejs.cn/ 上的才是想要的图。
...
- **核心框架**: **Flutter** (Dart)
  ...
  - *理由*: 完美的 Android/Windows/Web 跨平台能力，高性能渲染引擎支持复杂图表。

- **状态管理**: **Riverpod**。

- **本地存储**: **Isar**  (本地缓存，保证离线可用)。

- **文件存储（存储录音、录屏、图片）**: 文件系统+ [path_provider](https://pub.dev/packages/path_provider)。

  千万不要把音频/视频文件的二进制数据（Blob）直接存进数据库！这会让数据库体积爆炸，导致性能卡顿。

  **最佳实践流程：**

  1. **录音/录屏时**：使用 `path_provider` 获取 App 的本地沙盒目录。
     - Android/Windows: `getApplicationDocumentsDirectory()`
  2. **保存**：将文件命名为 `UUID.m4a` 或 `Timestamp.mp4` 存入该目录。
  3. **入库**：只在 Isar 数据库的 `Note` 对象中，存储这个文件的**路径字符串** (例如 `/data/user/0/.../rec_001.m4a`)。
  4. **同步**：后台任务读取这个路径，将文件上传到 Supabase Storage，上传成功后，在数据库里更新字段 `remoteUrl`。

- **配置存储**: [shared_preferences](https://pub.dev/packages/shared_preferences)。

  - 用于存储简单的键值对，与业务逻辑无关的数据，*场景*：是否开启深色模式、是否开启通知声音、上一次同步的时间戳。

- **多媒体处理**:

  - 录音: `record`
  - 录屏: `screen_recorder` (需配合 Platform Channel 开发 Windows 原生插件)

- **通知**：flutter_local_notifications。

- **可视化**：flutter_markdown + mermaid.js 渲染包 (e.g., flutter_mermaid)。

- **supabase 客户端库**：[supabase-flutter](https://pub.dev/packages/supabase_flutter)（与 Postgres 数据库交互、监听数据库更改、调用 Deno Edge 函数、构建登录和用户管理功能以及管理大型文件）。

#### 后端 (BaaS)

- **核心服务**: **Supabase**
  - **Auth**: Supabase Auth (管理多端登录，数据隔离)。
  - **Database**: PostgreSQL (SQL DB，存储结构化数据)。
  - **Storage**: Supabase Storage (对象存储，存储非结构化数据)。
  - **Edge Functions**: 让 Supabase 作为移动端和 ai 的中转层。
- *策略*: 优先使用云服务，未来可迁移至 Docker 自托管。

#### AI 服务

- **LLM 聚合**: **OpenRouter**
  - *模型策略*: 灵活切换 GPT-4o (复杂逻辑/图表生成) 和 Claude 3.5 Sonnet (长文本摘要)。
- **语音转文字 (STT)**: **OpenAI Whisper API**
  - *理由*: 性价比最高 ($0.006/min)，准确率优于手机端侧模型，不消耗手机电量。
...
客户端直接使用 Supabase SDK 调用 Database/Storage，AI 通过 Edge Functions。

通信方式:

- 直接使用 supabase_flutter SDK 调用 Database / Storage / Auth
- AI 相关操作通过 Supabase Edge Functions 中转调用（避免客户端暴露密钥）
- 所有接口均需经过 Supabase Auth 认证（JWT）

#### 2.4.1 认证相关

| 操作            | 方法  | 路径 / 函数名               | 说明                 | 返回字段示例                              |
| --------------- | ----- | --------------------------- | -------------------- | ----------------------------------------- |
| 登录 / 注册     | SDK   | supabase.auth.signInWith... | 支持 Email、OAuth 等 | session, user                             |
| 获取当前用户    | GET   | supabase.auth.getUser()     | 获取已登录用户信息   | { id, email, ... }                        |
| 更新用户 Prompt | PATCH | users 表 update             | 更新自定义 AI 提示词 | { prompt: "Always use bullet points..." } |

#### 2.4.2 Notes 资源

基路径：`supabase.from('notes')`

| 操作         | 方法   | 条件 / 参数示例                                              | 说明                             | 返回 / 影响字段                                      |
| ------------ | ------ | ------------------------------------------------------------ | -------------------------------- | ---------------------------------------------------- |
| 创建笔记     | INSERT | { user_id, content, type, local_path?, ... }                 | 新建一条笔记（通常先本地创建）   | id (UUID), created_at                                |
| 获取笔记列表 | SELECT | .eq('user_id', user.id) .eq('is_del', false) .order('created_at', { ascending: false }) | 获取当前用户未删除笔记（可分页） | 数组 [ {id, content, summary, tags, quadrant, ...} ] |
| 获取单条笔记 | SELECT | .eq('id', noteId) .eq('user_id', user.id)                    | 获取具体笔记详情                 | 单对象                                               |
| 更新笔记     | UPDATE | .eq('id', noteId) .update({ summary?, tags?, quadrant?, reminder_time?, mermaid_code?, content? }) | 更新 AI 处理结果或用户手动编辑   | 更新后的对象                                         |
| 软删除笔记   | UPDATE | .eq('id', noteId) .update({ is_del: true })                  | 标记删除（进入回收站）           | 无内容返回，状态码 204                               |
| 恢复笔记     | UPDATE | .eq('id', noteId) .update({ is_del: false })                 | 从回收站恢复                     | 无内容返回，状态码 204                               |
| 彻底删除     | DELETE | .eq('id', noteId)                                            | 仅管理员或回收站内操作（慎用）   | -                                                    |
...
supabase.from('notes')
  ...
  .eq('user_id', userId)
  .eq('is_del', false)
  .is('processed_at', null)           // 假设有 processed_at 字段表示已 AI 处理
...
supabase.from('notes')
  ...
  .eq('user_id', userId)
  .eq('is_del', false)
  .inFilter('quadrant', ['q1_urgent_important', 'q3_urgent_not_important'])
  .order('reminder_time', ascending: true);
```

#### 2.4.3 Flashcards 资源

基路径：`supabase.from('flashcards')`

| 操作           | 方法   | 条件 / 参数示例                                              | 说明                                     | 返回 / 影响字段 |
| -------------- | ------ | ------------------------------------------------------------ | ---------------------------------------- | --------------- |
| 创建卡片       | INSERT | { note_id, question, answer, srs_level: 0, next_review }     | AI 或手动创建抽认卡                      | id, next_review |
| 获取待复习卡片 | SELECT | .eq('user_id', user.id*) .lte('next_review', now()) .order('next_review') | 获取当前应该复习的卡片（*需 join notes） | 数组            |
| 更新复习结果   | UPDATE | .eq('id', cardId) .update({ srs_level, next_review })        | 根据用户反馈更新 SM-2 状态               | 更新后的卡片    |

**备注：**user_id 通常通过 JOIN notes 获取，或在 Edge Function 中自动填充。

#### 2.4.4 Edge Functions（AI 中转接口）

基路径：`supabase.functions.invoke('function_name', { body })`

| 函数名                 | 触发方式               | 输入参数（body）                                   | 主要功能                                   | 输出（response）示例                           | 异步回调方式            |
| ---------------------- | ---------------------- | -------------------------------------------------- | ------------------------------------------ | ---------------------------------------------- | ----------------------- |
| transcribe-audio       | 手动 / Storage webhook | { file_url: string }                               | 调用 Whisper API 转写音频                  | { success: true, text: "转写结果", note_id? }  | 更新 notes.content      |
| process-note           | 手动 / 转写完成后      | { note_id: string }                                | 完整 AI 处理：摘要、分类、象限、生成卡片等 | { success: true, updated_fields: {...} }       | 更新 notes 多字段       |
| generate-visualization | 手动 / process-note 后 | { note_id: string, style?: "mindmap"/"flowchart" } | 生成 Mermaid 代码并存入 mermaid_code       | { success: true, mermaid_code: "graph TD..." } | 更新 notes.mermaid_code |
| generate-flashcards    | 手动                   | { note_id: string, count?: number }                | 从笔记内容生成抽认卡                       | { success: true, created: number }             | 插入 flashcards 表      |
| soft-delete-note       | 手动                   | { note_id: string }                                | 安全软删除（可扩展为批量）                 | { success: true }                              | 更新 is_del = true      |
...
final response = await supabase.functions.invoke(
  'process-note',
  body: {'note_id': 'uuid-xxx-xxx'},
);
```

#### 2.4.5 Storage 接口（文件上传/下载）

桶名建议：notes-media（私有）

| 操作         | 方法                                     | 路径示例                            | 说明                              | 返回值                           |
| ------------ | ---------------------------------------- | ----------------------------------- | --------------------------------- | -------------------------------- |
| 上传文件     | storage.from('bucket').upload()          | 'user_id/note_id/recording-xxx.m4a' | 上传录音/录屏文件                 | publicUrl / signedUrl            |
| 获取签名 URL | storage.from('bucket').createSignedUrl() | 同上                                | 生成临时访问链接（用于播放/下载） | { signedUrl: "...", expires_at } |
| 删除文件     | storage.from('bucket').remove()          | 同上                                | 删除不再需要的媒体文件            | -                                |

推荐命名规则：

- 路径：`${user_id}/${note_id}/${type}/${timestamp}.${ext}`
- 示例：`abc123/note-456/audio/20260121_2325.m4a`
...
supabase
  ...
  .onPostgresChanges(
    ...
    callback: (payload) {
      ...
      if (payload.type == PostgresChangeType.delete) {
        // 检查 payload.oldRecord.is_del 是否为 true
      }
    },
  )
  .subscribe();
...
| HTTP 状态码 | 含义       | 常见场景                         | 建议客户端处理                |
| ----------- | ---------- | -------------------------------- | ----------------------------- |
| 401         | 未授权     | Token 过期 / 未登录              | 跳转登录页                    |
| 403         | 权限不足   | RLS 阻止（非本人数据）           | 显示“无权限”提示              |
| 429         | 频率限制   | Edge Function 调用过于频繁       | 显示“操作太快，请稍后重试”    |
| 500         | 服务端错误 | AI API 失败 / Edge Function 异常 | 显示友好错误 + “稍后重试”按钮 |

#### 2.4.8 安全与最佳实践

- 所有表启用 RLS：using (auth.uid() = user_id)

- Edge Functions 中校验用户身份：const user = await getUserFromHeader(req.headers.authorization)

- 敏感操作（删除、更新 prompt）需二次确认

- 文件上传大小限制建议：单文件 ≤ 100MB（Whisper API 支持）

- Prompt 字段长度限制：建议 ≤ 1000 字符

#### 2.4.9 Supabase SDK 调用示例 (Dart)

- 上传录音：`supabase.storage.from('bucket').upload('path', file);` 然后调用 Edge Function `supabase.functions.invoke('transcribeAudio', body: {'file_url': url});`
- 查询 notes：`supabase.from('notes').select().eq('user_id', user.id).eq('is_del', false).order('created_at', ascending: false);`
- 软删除：`supabase.from('notes').update({ is_del: true }).eq('id', note_id);`
- 更新用户 Prompt：`supabase.from('users').update({ prompt: new_prompt }).eq('id', user.id);`
- 订阅更新：`supabase.from('notes').stream(primaryKey: ['id']).listen((data) => updateUI(data));`
...
- **性能**：Isar 查询优化 (索引)，Flutter 列表使用 SliverList 避免内存溢出。
- **测试**：单元/集成/端到端测试。
- **扩展**：Windows 适配使用 desktop_webview_authentication for Auth；录屏用 Win32 API via Platform Channels。
- **风险**：AI 延迟 - 解决方案：客户端显示“处理中”状态，异步轮询结果；用户 Prompt 恶意注入 - 解决方案：Edge Functions 中 sanitization 或长度限制。
...
```text
1. 测试计划：测试范围、策略、资源、进度。

2. 测试用例：详细描述测试步骤、预期结果。
...
- **1. 部署文档/运维手册**：详细的部署步骤、回滚方案、服务器配置。
- **2. 监控与告警系统**：如 Prometheus + Grafana 的仪表盘，监控服务器 CPU、内存、错误日志、业务指标。
- **3. 用户手册/帮助文档**：面向最终用户的操作指南。
- **4. 上线发布清单**：确保发布前所有准备工作就绪。
- **5. 用户反馈渠道与分析**：应用内反馈、应用商店评论、用户行为分析数据。
- **6. 迭代需求清单**：基于反馈规划的下一版本功能列表。
...
- **问题**: Supabase 涨价或服务变更。
- **策略**:
  - 数据库：使用 `pg_dump` 导出标准 SQL，可迁移至任何 Postgres 服务。
  - 文件：编写脚本批量下载。
  - 自托管：使用 Docker 在廉价 VPS 上部署 Supabase Self-hosted 版本。

### 8.2 成本控制

- **策略**:
  - MVP 阶段：全量使用 Whisper API (成本低，开发快)。
  - 增长阶段：若日均录音时长爆炸，引入 VAD (静音检测) 减少上传时长。
  - 极端情况：仅在高端 Android 机型上启用本地 `whisper.cpp` 模型作为备选。
